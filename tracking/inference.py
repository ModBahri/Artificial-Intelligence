    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for old in self.legalPositions:
            newdistr = self.getPositionDistribution(self.setGhostPosition(gameState, old))
            for new, prob in newdistr.items():
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for legalPos in self.legalPositions:
                trueDist = util.manhattanDistance(legalPos, pacmanPosition)
                allPossible[legalPos] = allPossible[legalPos] + (emissionModel[trueDist] * oldBelief[legalPos])
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            tmpPart.append(util.sample(newPosDist))
        self.particles = tmpPart
